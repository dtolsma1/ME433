...
// Timer2 delay times, you can tune these if necessary (lower by 1 to account)
...
void ws2812b_setup() {
    ...
    TRISBbits.TRISB6 = 0;   //B6 = output
    LATBbits.LATB6 = 0;     //B6 output off to start
}
...
void ws2812b_setColor(wsColor * c, int numLEDs) {
    ...
    for (i = 0; i < numLEDs; i++) {
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if (((c[i].r >> j) & 0x01) == 0x01) { /* identify the bit in c[].r, is it 1 */
                // the high is longer
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if (((c[i].g >> j) & 0x01) == 0x01) { /* identify the bit in c[].r, is it 1 */
                // the high is longer
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME;
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if (((c[i].b >> j) & 0x01) == 0x01) { /* identify the bit in c[].r, is it 1 */
                // the high is longer
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME;
...
void ws2812b_setAllSame(wsColor *c, unsigned char red, unsigned char green, unsigned char blue) {
    int i;
    for (i = 0; i < NUMLEDS; i++) {
        ws2812b_initColor(c, i, red, green, blue);
    }
